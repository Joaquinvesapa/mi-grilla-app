...
// ── LiveNowMenu (main export) ──────────────────────────────

/**
 * Global "EN VIVO" overlay: a FAB that opens a bottom sheet showing
 * per-stage live and upcoming artist info. Mounts in the app layout
 * as a sibling to BottomNav. Data comes from useLiveNow() hook
 * which reads from IDB cache — fully offline-capable.
 */
export function LiveNowMenu() {
  const { liveStages, isLive, isLoading } = useLiveNow();
  const [isOpen, setIsOpen] = useState(false);

  const closeButtonRef = useRef<HTMLButtonElement | null>(null);
  const fabRef = useRef<HTMLButtonElement | null>(null);

  // ── Determine if there are any stages with content ──────
